...
* instructions/references/instanceof.go checkcast.go - `instanceof` 指令
* instructions/references/checkcast.go - `checkcast` 指令
* instructions/arrays/newarray.go - 加入 `anewarray` 指令
...
* 陣列類型系統不完整：完整實現需要動態生成陣列類別
    * 完整的 JVM 陣列類型系統需要：
        * 當遇到 String[] 時，JVM 需要動態生成 "[Ljava/lang/String;" 類別
        * 這個類別的父類是 Object
        * 實現 Cloneable 和 Serializable 介面

* aastore 類型檢查：目前不檢查元素類型相容性
    * 當執行 arr[i] = obj 時，需要檢查 obj 的類型是否與陣列元素類型相容
    * 例如：Object[] arr = new String[5]; arr[0] = new Integer(1); // 編譯通過，但執行時拋 ArrayStoreException

* 陣列協變：未處理 Object[] arr = new String[5] 的情況
    * String[] 可以賦值給 Object[]（因為 String 是 Object 的子類）
    * 但這會導致上述的 ArrayStoreException 風險

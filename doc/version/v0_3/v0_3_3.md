...
* runtime/method_area/class_lifecycle.go: 新增 `IsCloneable()` 和 `HasNonTrivialFinalizer()` 方法
* runtime/heap/finalizer.go: Finalization Queue 機制（為 v0.5.x GC 做準備）

<br>

### 修改檔案

* native/java/lang/object.go: 完善 `clone()` 實現，加入 `Cloneable` 介面檢查
